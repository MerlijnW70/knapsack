...
fn new_writes_always_produce_ks2_and_match_expected_shape() {
    ...
    for b in inputs {
        ...
        assert!(
            h.starts_with("ks2_"),
            "new writes start with ks2_; got `{}` for input of {} bytes",
            h,
            b.len()
        );
...
fn legacy_handle_with_wrong_bytes_is_rejected_by_verify() {
    ...
    let s = Store::new(legacy_dir.clone());
    assert!(
        s.get(&legacy_handle).is_none(),
        "corrupted legacy file must read as None, never as wrong bytes"
    );
    let _ = std::fs::remove_dir_all(&legacy_dir);
}
...
fn is_valid_handle_accepts_both_formats_and_rejects_junk() {
    // Whitelist
    assert!(
        is_valid_handle("ks2_00112233445566778899aabbccddeeff"),
        "32-hex ks2 ok"
    );
...
fn verify_round_trips_both_formats() {
    ...
    assert!(
        verify(&legacy10, payload),
        "legacy 10-hex verifies via SHA-1"
    );
    let legacy16 = format!("ks_{}", &sha1_hex(payload)[..16]);
    assert!(
        verify(&legacy16, payload),
        "legacy 16-hex verifies via SHA-1"
    );
}
...
fn cli_expand_rejects_malformed_handle() {
    ...
    assert!(
        ...
        "must say `invalid handle`, not `no such handle`:\nstderr={}",
        stderr
    );
