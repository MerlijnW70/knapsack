...
fn text_of(resp: &str) -> String {
    ...
    if let Json::Arr(a) = content {
        a[0].get("text").and_then(|t| t.as_str()).unwrap().to_string()
    } else {
        panic!("no content")
    }
}
...
fn mcp_protocol_and_tools() {
    ...
    assert_eq!(v.get("result").and_then(|x| x.get("protocolVersion")).and_then(|x| x.as_str()), Some("2024-11-05"));
    assert_eq!(
        v.get("result").and_then(|x| x.get("serverInfo")).and_then(|x| x.get("name")).and_then(|x| x.as_str()),
        Some("knapsack")
    );
    ...
    assert!(r.contains("knapsack_expand") && r.contains("knapsack_inspect") && r.contains("knapsack_metrics"));
    ...
    assert_eq!(text_of(&handle_message(&call).unwrap()), "alpha\nbeta\ngamma\ndelta");
