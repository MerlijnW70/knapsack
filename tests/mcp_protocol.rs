...
fn mcp_protocol_conformance_edges() {
    ...
    let v =
        json::parse(&handle_message(r#"{"id":"abc-1","method":"tools/list"}"#).unwrap()).unwrap();
    assert_eq!(v.get("id").and_then(|x| x.as_str()), Some("abc-1"));
    let v = json::parse(&handle_message(r#"{"method":"initialize"}"#).unwrap()).unwrap();
    assert!(
        matches!(v.get("id"), Some(Json::Null)),
        "absent id -> null in the response"
    );
    ...
    assert!(
        handle_message(r#"{"id":2,"method":"tools/call","params":{"arguments":{}}}"#)
            .unwrap()
            .contains("-32601")
    );
    assert!(handle_message(r#"{"id":3,"method":"tools/call"}"#)
        .unwrap()
        .contains("-32601"));

    // expand: missing required handle, and wrong-typed handle -> isError text result.
    let r = handle_message(
        r#"{"id":4,"method":"tools/call","params":{"name":"knapsack_expand","arguments":{}}}"#,
    )
    .unwrap();
    ...
    assert!(
        r.contains("\"isError\":true"),
        "numeric handle (wrong type) -> treated as missing, not a panic"
    );
    ...
    assert!(
        r.contains("\"isError\":true") && r.contains("'lines'"),
        "malformed lines must isError: {r}"
    );
    ...
    assert!(
        r.contains("\"isError\":true") && r.contains("'context'"),
        "wrong-typed context must isError: {r}"
    );
    ...
    assert!(
        r.contains("\"isError\":true") && r.contains("'context'"),
        "negative context must isError: {r}"
    );

    // metrics tool with empty args is fine.
    assert!(handle_message(
        r#"{"id":6,"method":"tools/call","params":{"name":"knapsack_metrics","arguments":{}}}"#
    )
    .unwrap()
    .contains("knapsack live stats"));
    ...
    for p in probes {
        ...
        let v = json::parse(&resp)
            .unwrap_or_else(|e| panic!("server emitted invalid JSON for {p}: {e}"));
        assert_eq!(
            v.get("jsonrpc").and_then(|x| x.as_str()),
            Some("2.0"),
            "every response is jsonrpc 2.0: {resp}"
        );
