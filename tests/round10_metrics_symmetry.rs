...
fn negative_net_session_renders_identically_on_mcp_and_cli() {
    ...
    metrics::record_expand("over-recall", "ks2_test", 300, true, metrics::ExpandMode::Whole, metrics::ExpandCaller::Cli);
...
fn huge_numeric_values_render_identically() {
    ...
    metrics::record_expand("whale", "ks2_test", 999_999_999, true, metrics::ExpandMode::Whole, metrics::ExpandCaller::Cli);
...
fn failed_expand_attribution_renders_identically_on_both_surfaces() {
    ...
    metrics::record_expand("flaky", "ks2_test", 50, true, metrics::ExpandMode::Whole, metrics::ExpandCaller::Cli);   // ok
    metrics::record_expand("flaky", "ks2_test", 75, false, metrics::ExpandMode::Whole, metrics::ExpandCaller::Cli);  // fail
    metrics::record_expand("flaky", "ks2_test", 25, true, metrics::ExpandMode::Whole, metrics::ExpandCaller::Cli);   // ok
...
fn many_sessions_sort_order_is_identical_across_surfaces() {
    ...
    for i in 0..10usize {
        ...
        if i.is_multiple_of(2) {
            metrics::record_expand(&sid, "ks2_test", i * 10, true, metrics::ExpandMode::Whole, metrics::ExpandCaller::Cli);
        }
    }
