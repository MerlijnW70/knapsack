...
fn code_extension_wins_regardless_of_content() {
    for ext in ["rs", "js", "jsx", "ts", "tsx", "py", "go", "java", "c", "h", "cpp", "rb", "php", "swift", "kt"] {
        let path = format!("src/thing.{ext}");
        assert_eq!(
            detect(b"this is not even code\njust some words here\n", Some(&path)),
            ContentType::Code,
...
fn obvious_log_content_without_extension_is_log() {
    ...
    for i in 0..40 {
        s.push_str(&format!("2026-05-25T10:{:02}:00 request id={i} latency={}ms status=200\n", i % 60, i * 3));
    }
...
fn too_little_signal_defaults_to_log() {
    ...
    assert_eq!(detect(b"def f():\n    pass\n", None), ContentType::Log, "2 lines is not enough to call it code");
...
fn binary_and_adversarial_never_panic() {
    ...
    let mixed: Vec<u8> = b"\x00\xff\n\xfe\x80line\n".iter().cycle().take(3000).copied().collect();
